...
let uvmapchecker = texLoader.load("Textures/uvchecker.png");
uvmapchecker.wrapS = uvmapchecker.wrapT = THREE.RepeatWrapping;
uvmapchecker.anisotropy = 16;
uvmapchecker.repeat.set(1, 1)
let wWidth = 2.5,
  wHeight = 6,
  wDepth = 1.5,
  segment_id = [],
...
let _lockers = [],
  ...
  internalDrawersInterior_parent = [],
  internalDrawerInteriors = [],
...
let _splitterMaterial,
  ...
  _wardrobeLMaterial,
  _wardrobeRMaterial,
  _wardrobeTopMaterial,
  _wardrobeBotMaterial,
  _wardrobeVMaterial,
  _lockerMaterial,
  _shelfMaterial,
  _hangerMaterial,
...
async function getTextures() {
  fetch("colorinfo.json")
    ...
    .then((data) => {
      textures = data;
      for (let i in textures.metal) {
        textures.metal[i].albedo = texLoader.load(data.metal[i].albedo);
        textures.metal[i].metallic = texLoader.load(data.metal[i].metallic);
        textures.metal[i].roughness = texLoader.load(data.metal[i].roughness);

        updateTexture(textures.metal[i].albedo,1,1,true);
        updateTexture(textures.metal[i].metallic,1,1,true);
        updateTexture(textures.metal[i].roughness,1,1,true);

      }
      for (let i in textures.wood) {

        textures.wood[i].albedo = texLoader.load(data.wood[i].albedo);
        textures.wood[i].normal = texLoader.load(data.wood[i].normal);
        textures.wood[i].ao = texLoader.load(data.wood[i].ao);
        textures.wood[i].height = texLoader.load(data.wood[i].height);
        textures.wood[i].roughness = texLoader.load(data.wood[i].roughness);

        updateTexture(textures.wood[i].albedo);
        updateTexture(textures.wood[i].normal);
        updateTexture(textures.wood[i].ao);
        updateTexture(textures.wood[i].height);
        updateTexture(textures.wood[i].roughness);
...
function setLighting() {
  ...
  new THREE.CubeTextureLoader(manager).load(urls, function (cubeTexture) {
    ...
    lightProbe.copy(
      THREE.LightProbeGenerator.fromCubeRenderTarget(renderer, roomCubeMap)
    );
...
function create_lights() {
  // setLighting();
  const areaLight = new THREE.RectAreaLight(0xdedede, 1, 20, 20);
  areaLight.position.set(0, 20 * ftTom, 5);
  areaLight.rotation.x = -Math.PI / 2;
  // scene.add(areaLight);

  scene.add(new THREE.AmbientLight(0xffffff, 1));

  const light = new THREE.DirectionalLight(0xababab, 2);
  ...
  scene.add(light2);
  var bulbLight = new THREE.PointLight(0xfdfdfd, 1, 25, 1);
  ...
  // var bulbLight2 = new THREE.PointLight(0xdedede, 2, 100, 1);
  ...
  // const d = 100;
  // bulbLight2.shadow.camera.bottom = d;
  // bulbLight2.shadow.camera.top = d;
  // bulbLight2.shadow.camera.left = d;
  // bulbLight2.shadow.camera.right = d;
  // bulbLight2.shadow.camera.far = 200;
...
function updateLoftDoorsHandles(index) {
  if (loftDoorHandles[index] instanceof THREE.Object3D) {
    ...
    for (let i in loftDoorHandles[index].children) {
      ...
      d.position.set(
        parent.position.y + parent.children[0].position.y,
        -offset + (2 * ftTom) / 12,
        0
      );
...
function createWardrobe() {
  ...
  var t = g.clone();
  t.rotateY(Math.PI / 2)
  wBottom = new THREE.Mesh(t, _wardrobeBotMaterial);
  ...
  wBottomLayer = new THREE.Mesh(t, _wardrobeBotMaterial);
...
function updateWardrobe() {
  ...
  if (wBack) {
    ...
    wBack.position.setY(
      wBack.scale.y / 2 + wBottom.position.y - wBottom.scale.y / 2
    );

    if (isHingedDoor) {
      wBack.position.setZ(-wBottom.scale.z / 2);
    } else {
      wBack.position.setZ(-wBottom.scale.z / 2 - (1.35 / 24) * ftTom);
    }
          ...
          if (d instanceof THREE.Object3D) {
            if (d.name.startsWith("ext")) {
              ...
              if (index % 2 == 0) {
                d.rotation.y = 0;
                d.position.setZ(
                  parent.children[0].position.z +
                  parent.children[0].scale.z / 2 +
                  d.scale.z / 2
                );
              } else {
                d.rotation.y = Math.PI;
...
function updateHingedDoorUpSize(index) {
  if (_hDoors_parent[index] instanceof THREE.Group) {
    ...
    if (_extDrawers_splitters.length > 0) {
      ...
      posY =
        wBack.scale.y / 2 + wBottom.position.y - wBottom.scale.y + wTop.scale.y;
      scaleY =
        wTop.position.y -
        wTop.scale.y / 2 -
        wBottom.position.y -
...
function initMaterial() {
  ...
  //4 back
  //5 front
  colormat = new THREE.MeshStandardMaterial({
    color: defaultColor,
    name: "color",
  });
  texmat = new THREE.MeshStandardMaterial({
    color: 0xffffff,
    name: "texture",
    map: uvmapchecker,
  });
  var w_materials = [colormat, colormat, colormat, colormat, texmat, texmat];
  var side_materials_left = [
    colormat,
    texmat,
    texmat,
    colormat,
  ...
  var mid_materials = [colormat, colormat, texmat, texmat, colormat, colormat];
  var bot_materials = [colormat, colormat, colormat, texmat, texmat, texmat];
  var top_materials = [colormat, colormat, texmat, colormat, texmat, texmat];
  _wardrobeMaterial = w_materials;
  _wardrobeTopMaterial = top_materials;
  _wardrobeBotMaterial = bot_materials;
  _wardrobeLMaterial = side_materials_left;
  _wardrobeRMaterial = side_materials_right;

  _doorMaterial = w_materials;

  _columnsMaterial = mid_column_materials;
  _shelfMaterial = mid_materials;
  _splitterMaterial = mid_materials;
...
function createInternalDrawersInterior(index, row) {
  if (internalDrawer != null) {
    let internalDrawersInterior_group = new THREE.Group();

    for (var j = 0; j < row; j++) {
      var drawer = internalDrawer.clone();
      internalDrawerInteriors[j] = drawer;
      internalDrawersInterior_group.add(internalDrawerInteriors[j]);
    }
    internalDrawersInterior_parent[index] = internalDrawersInterior_group;

    scene.add(internalDrawersInterior_parent[index]);
  }
}

function updateInternalDrawersInterior(index) {
  if (internalDrawersInterior_parent[index] instanceof THREE.Group) {
    internalDrawersInterior_parent[index].position.copy(
      _intDrawers_parent[index].position
    );
    for (let j in internalDrawersInterior_parent[index].children) {
      if (_intDrawers_parent[index].children[j] instanceof THREE.Mesh) {
        var parent = _intDrawers_parent[index].children[j];
        var parentSize = new THREE.Box3()
          .setFromObject(parent)
          .getSize(new THREE.Vector3());
        var drawer = internalDrawersInterior_parent[index].children[j];
        drawer.position.copy(parent.position);
...
function removeInternalDrawersInterior(index) {
  if (!index) {
    internalDrawersInterior_parent.forEach((e) => {
      if (e instanceof THREE.Group) {
        scene.remove(e);
      }
    });
    ...
    internalDrawersInterior_parent.forEach((e) => {
      if (
        ...
        index == internalDrawersInterior_parent.indexOf(e)
      ) {
        scene.remove(e);
      }
    });
    internalDrawersInterior_parent[index] = null;
  }
}
  ...
  if (room != null) {
    ...
    room.traverse((e) => {
      if (e instanceof THREE.Mesh) {
        ...
        if (
          mat.name.includes("[Metal_Corrogated_Shiny]5") ||
          mat.name.includes("[Metal_Corrogated_Shiny]4")
        ) {
    ...
    room.traverse((e) => {
      if (e instanceof THREE.Object3D) {
        if (e.name.includes("Minutes")) {
          e.rotation.z = -((d.getMinutes() * Math.PI) / 30) +
            (d.getSeconds() * Math.PI) / (30 * 60);
...
function updateTexture(tex = new THREE.Texture(), x = 1, y = 1,mirrorrepeat= false) {
  if (tex != null) {
...
function rotateTo(obj, to) {

  var initial = obj.rotation;
  var value = to;
...
function setWoodColor(material, type) {

  if (type == 0 || type == 1) {
    switch (Math.round(colorType)) {
      case 0:
        colorTo(material, textures.colors.offwhite);
        break;
      case 1:
        colorTo(material, textures.colors.grey);
        break;
      case 2:
        colorTo(material, textures.colors.lightbrown);
        break;
      case 3:
        colorTo(material, textures.colors.darkbrown);
        break;
      case 4:
        colorTo(material, textures.colors.black);
        break;

    }
  } else if (type == 2 || type == 3) {
    switch (Math.round(woodColor)) {
      case 0:
        colorTo(material, textures.tint.natural);
        break;
      case 1:
        colorTo(material, textures.tint.cherry);
        break;
      case 2:
        colorTo(material, textures.tint.walnut);
        break;
    }
  }
...
function updateDrawerMaterial(map,normalMap,aoMap,roughnessMap,bumpMap,roughness = 1,normalScale = new THREE.Vector2(2,2),bumpScale=1 ){
  scene.traverse(a => {
    if (a instanceof THREE.Mesh) {
          if(a.material instanceof THREE.MeshStandardMaterial){
            
            if(a.material.name.startsWith("drawerWood")){
                 var e = a.material;
                setWoodColor(e, materialType)
                ...
                e.aoMapIntensity = 1;
                e.normalScale = normalScale
                e.roughness = roughness;
                e.bumpMap = bumpMap;
                e.bumpScale = bumpScale;
...
function updateMatTexture(material, x = 1, y = 1, rotation = 0) {
  try {
    var normalScale = new THREE.Vector2(2,2)
    if (materialType == 0 || materialType == 1) {
      ...
      if (texmat instanceof THREE.MeshStandardMaterial) {
        var e = texmat;
        setWoodColor(e, materialType)
      ...
      if (colormat instanceof THREE.MeshStandardMaterial) {
        var e = colormat;
        setWoodColor(e, materialType)
      ...
      updateDrawerMaterial(null,null,null,null,null,1,normalScale,1)
      ...
      if (texmat instanceof THREE.MeshStandardMaterial) {
        ...
        e.map = setWoodTexture()[0];
    
        e.normalMap = setWoodTexture()[1];
        e.aoMap = setWoodTexture()[2];
        e.roughnessMap = setWoodTexture()[3];
        e.aoMapIntensity = 2;
        e.normalScale = normalScale
        e.roughness = 1;
        e.bumpMap = setWoodTexture()[4];
      }
      if (colormat instanceof THREE.MeshStandardMaterial) {
        var e = colormat;
      ...
      updateDrawerMaterial(setWoodTexture()[0],setWoodTexture()[1],setWoodTexture()[2],setWoodTexture()[3],setWoodTexture()[4],0.5,normalScale,1)
...
function setWoodTexture() {
  

  return [
    textures.wood[woodTexture].albedo,
    textures.wood[woodTexture].normal,
    textures.wood[woodTexture].ao,
    textures.wood[woodTexture].roughness,
    textures.wood[woodTexture].height
  ];
...
function getMetalColor() {
      ...
      return [
        textures.metal[handleColor].albedo,
        textures.metal[handleColor].roughness,
        textures.metal[handleColor].metallic,
      ];
...
function scaleUV(obj, x, y) {
  try {
    if (obj != null) {
      ...
      var factorX = (x / 2.5) * 0.25;
      var factorY = (y / 2.5) * 0.25;
      for (var i = 0; i < uvAttribute.count; i++) {
      ...
      handles.traverse((e) => {
        if (e instanceof THREE.Mesh) {
          if (e.material.name.includes("handles")) {
            ...
            e.material.roughnessMap.repeat.set(2, 2);
            e.material.metalnessMap = getMetalColor()[2];
            e.material.metalness = 0.8;
         
            e.material.metalnessMap.repeat.set(2, 2);
          }
        }
      });
    }
...
function updateMaterials() {
  ...
  //Wardrobe Back
  var scaleFactor = 4;
  var wFactor = wWidth * scaleFactor;
  var hFactor = wHeight * scaleFactor;
  var dFactor = wDepth * scaleFactor;

  updateMatTexture();


  scaleUV(wBack, wFactor, hFactor)
  scaleUV(wpLoftBack, wFactor, hFactor)



  scaleUV(wTop, dFactor, wFactor);
  scaleUV(wTopLayer, dFactor, wFactor);
  scaleUV(wBottom, dFactor, wFactor);
  scaleUV(wBottomLayer, dFactor, wFactor);
  scaleUV(wpLoftTop, dFactor, wFactor);
  scaleUV(wpLoftBottom, dFactor, wFactor);


  scaleUV(wLeft, dFactor, hFactor);
  scaleUV(wLeftLayer, dFactor, hFactor);
  scaleUV(wLeftLayerBottom, dFactor, hFactor);
  scaleUV(wLeftLayerTop, dFactor, hFactor);
  scaleUV(wRight, dFactor, hFactor);
  scaleUV(wRightLayerTop, dFactor, hFactor);
  scaleUV(wRightLayerTop, dFactor, hFactor);
  scaleUV(wRightLayerTop, dFactor, hFactor);
  scaleUV(wpLoftRight, dFactor, hFactor);
  scaleUV(wpLoftLeft, dFactor, hFactor);
...
function updateStandingPlane() {
  if (standingPlane) {
    ...
    standingPlane.position.set(
      wLeft.position.x - standingPlane.scale.x / 2 - 0.2,
      standingPlane.scale.y / 2,
      wLeft.scale.z / 2
    );
  }
}
